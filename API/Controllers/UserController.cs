...
public class UserController : ControllerBase
{
    ...
    public async Task<IActionResult> GetUser([FromQuery] string? username)
    {
        try
        {
            ...
            return user switch
            {
                null => throw new UserNotFoundException(username),
                _ => Ok(
    ...
    private static readonly Regex _sUserNameAllowedRegEx = new(
            @"^[a-zA-Z]{1}[a-zA-Z0-9\._\-]{0,23}[^.-]$",
            RegexOptions.Compiled
        );
    private static readonly Regex _sUserNameIllegalEndingRegEx = new(
            @"(\.|\-|\._|\-_)$",
            RegexOptions.Compiled
        );
    private static bool IsValidUsername(string userName)
    {
        if (string.IsNullOrEmpty(userName)
            || !_sUserNameAllowedRegEx.IsMatch(userName)
            || _sUserNameIllegalEndingRegEx.IsMatch(userName))
            return false;

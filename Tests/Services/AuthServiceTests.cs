...
public class AuthServiceTests
{
    ...
    public async Task RegisterAsync_Returns_WhenSuccess()
    {
        ...
        List<string> userRoles = new() { Roles.Admin, Roles.User };
    ...
    public async Task LoginAsync_Returns_WhenSuccess()
    {
        ...
        _userManager.Setup(x => x.CheckPasswordAsync(_user, password)).ReturnsAsync(true);

        _unitOfWork.Setup(x => x.TokenRepository.AddAsync(It.IsAny<RefreshToken>()));

        LoginModelRequest registrationModelRequest = new()
        {
            Email = _user.Email,
            Password = password,
        };
